...
void priority_enqueue(queue_t q, void* element){
    ...
    request_t request = (request_t)((task_t)element)->args;
    
    int size = request->file_size;
    //printf("file_size input:%d\n", size);
    task_t t = (task_t)element;
    new->element = element;
    new->next = NULL;
    while(tmp->next){
        request_t next = (request_t)((task_t)tmp->next->element)->args;
...
void display(queue_t q){
    struct node* tmp = q->front->next;
    while(tmp){
        request_t re = (request_t)((task_t)tmp->element)->args;
        printf("element: %d,",re->file_size);
        tmp = tmp->next;
    }
    printf("\n");
}
